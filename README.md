...
For logging/debugging, you can specify a custom key from payload:

```typescript
// Shorthand - directly pass function
...
|-----------|-------------|
| `workflow.name` | Workflow class name or custom name |
| `workflow.instance_id` | Cloudflare workflow instance ID |
| `workflow.key` | Deterministic key from payload |
...
|----------|-------------|
| `@traceWorkflow(opts?)` | Decorator for auto-tracing workflow class |
| `@traceWorkflow(keyFn)` | Shorthand with key extractor function |
| `withWorkflowTrace(payload)` | Inject `_traceparent` into workflow payload |
| `generateDeterministicKey(value)` | Generate consistent key from value (for debugging) |

**TraceWorkflowOptions:**

| Option | Type | Description |
|--------|------|-------------|
| `key` | `(event) => unknown` | Extract key from event for logging |
| `name` | `string` | Custom workflow name for span |
...
Since we use `@opentelemetry/api` interfaces, in the future you can:
1. Add OTLP exporter via `fetch()` + `waitUntil()`
2. Or use Cloudflare Destinations to export native traces

### Cloudflare Native Traces

Keep `[observability.traces]` enabled in `wrangler.toml`:
...
Native traces and this library are complementary:
- **Native traces**: Auto-instrument I/O (D1, KV, R2, fetch)
- **This library**: Log correlation & trace propagation across services
